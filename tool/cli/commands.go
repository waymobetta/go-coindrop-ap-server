...
type (
	...
	// SendPaycheckCommand is the command line data structure for the send action of paycheck
	SendPaycheckCommand struct {
		Payload     string
		ContentType string
		PrettyPrint bool
	}
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "send",
		Short: `Sends ethereum wallet token amount`,
	}
	tmp1 := new(SendPaycheckCommand)
	sub = &cobra.Command{
		Use:   `paycheck ["/v1/paycheck"]`,
		Short: ``,
		Long: `
...

{
   ...
   "tokenAmount": 8478115314176678374
}`,
	...
	command = &cobra.Command{
		Use:   "show",
		Short: `Returns OK if system is healthy`,
	}
...
// Run makes the HTTP request corresponding to the SendPaycheckCommand command.
func (cmd *SendPaycheckCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	var payload client.PaycheckPayload
	if cmd.Payload != "" {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.SendPaycheck(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *SendPaycheckCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
